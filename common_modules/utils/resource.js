    ...
    if (fullPath && !_s.startsWith(fullPath, errorPage)) {
      ...
      if ((!cResource || _.isEmpty(cResource) || !(fullPath === cResource.path)) || !event) {
        ...
        if (!_.isEmpty(currentResource) && currentResource.status !== "1") {
          obj.$router.push({path: errorPage, query: {status: currentResource.status}});
        } else if (path) {
  ...
  generateMenus(resources) {
    ...
    _.each(resources[constant.resource_type.system], menu => {
      if (menu.parentId === '0' || !menu.parentId) {
        menu.children = getChildren(menu.id, resources[constant.resource_type.system]);
    ...
    _.each(resources[constant.resource_type.link], menu => {
      if (menu.parentId === '0' || !menu.parentId) {
        menu.children = getChildren(menu.id, resources[constant.resource_type.link]);
        linkMenus.push(menu);
      }
    });
    ...
    _.each(resources[constant.resource_type.external], menu => {
      if (menu.parentId === '0' || !menu.parentId) {
        menu.children = getChildren(menu.id, resources[constant.resource_type.external]);
    ...
    _.each(resources[constant.resource_type.other], menu => {
      if (menu.parentId === '0' || !menu.parentId) {
        menu.children = getChildren(menu.id, resources[constant.resource_type.other]);
