...
(function ($) {
	...
	$.fn.wPaint.defaultSettings = {
		...
		menu: ['undo', 'redo', 'rectangle', 'ellipse', 'line', 'pencil', 'text', 'strokeColor'], // menu items - appear in order they are set
		menuOrientation: 'horizontal',		// orinetation of menu (horizontal, vertical)
		menuOffsetX: 110,					// offset for menu (left)
		menuOffsetY: 7,					// offset for menu (top)
		imageHeight: 0,
		imageWidth: 0,
	...
	Canvas.prototype =
		{
			...
			generate: function (width, height) {
				...
				var _self = this;
				var x = $(window).width() > _self.settings.imageWidth ? ($(window).width() - _self.settings.imageWidth) / 2 : 0;
				var y = $(window).height() > _self.settings.imageHeight ? ($(window).height() - _self.settings.imageHeight) / 2 : 0;
			...
			generateTextInput: function () {
				...
				_self.textInput =
					$('<textarea class="_wPaint_textInput" spellcheck="false"></textarea>')
						.css({ display: 'none', position: 'absolute', color: '#000', fontSize: this.settings.fontSize, lineHeight: this.settings.fontSize + 'px', fontFamily: this.settings.fontFamily })
			...
			drawShapeUp: function (e, _self) {
				var x = $(window).width() > _self.settings.imageWidth ? ($(window).width() - _self.settings.imageWidth) / 2 : 0;
				var y = $(window).height() > _self.settings.imageHeight ? ($(window).height() - _self.settings.imageHeight) / 2 : 0;
				x = _self.canvasTempLeftNew - x;
				y = _self.canvasTempTopNew - y;
				if (_self.settings.mode != 'Text') {
					_self.ctx.drawImage(_self.canvasTemp, x, y);
			...
			drawTextUp: function (e, _self) {
				...
				var scrollTop = $("#canvas_doodle").scrollTop();
				var scrollLeft = $("#canvas_doodle").scrollLeft();
				var x = $(window).width() > _self.settings.imageWidth ? ($(window).width() - _self.settings.imageWidth) / 2 : 0;
				var y = $(window).height() > _self.settings.imageHeight ? ($(window).height() - _self.settings.imageHeight) / 2 : 0;
				var left = offset.left + scrollLeft - x;
				var top = offset.top + scrollTop - y;
				var underlineOffset = 0;

				for (var i = 0, ii = lines.length; i < ii; i++) {
					_self.ctx.fillStyle = _self.settings.fontColor;
	...
	TextMenu.prototype =
		{
			generate: function (canvas, mainMenu) {
				...
				return this.menu =
					$('<div class="_wPaint_menu _wPaint_menu_' + $canvas.settings.menuOrientation + '""></div>')
						.css({ display: 'none', position: 'fixed', right: 194, marginTop: 5 })
						.draggable({
							snap: '._wPaint_menu',
							handle: menuHandle,
							stop: function () {
