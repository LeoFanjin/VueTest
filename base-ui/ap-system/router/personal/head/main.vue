    ...
    methods: {
      ...
      removeHead() {
        ...
        if (self.head.src === self.$store.state.default_head_img) {
          self.$message.warning(self.$t('userHead.msg.no_remove'));
        } else {
          self.$confirm(self.$t('common.label.sure_remove'), self.$t('common.label.prompt'), {
            ...
            self.$store.dispatch("removeUserHead", {}).then(function () {
              self.$message.success(self.$t('userHead.msg.head_update_success'));
              self.initHead();
              self.$store.dispatch("setUserHead", '');

            }).catch(function (message) {
              self.$message.error((message && typeof message === "string") ? message : self.$t('common.msg.delete_fail'));
            });
          }).catch(function () {
          });
        }
      ...
      uploadedSuccess(res) {
        ...
        if (!_.isEmpty(res)) {
          Ajax.dealSuccess(res).then(function (data) {
            ...
            self.$message.success(self.$t('userInfo.msg.head_update_success'));
            self.$store.dispatch("setUserHead", data);
          }).catch(function (error) {
            self.$message.error((error && typeof error === 'string') ? error : '上传失败');
          });
        } else {
          self.$message("上传失败");
        }
